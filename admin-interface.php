...
<div class="wrap">
	...
	<form id="outpost" method='POST' action="" enctype="multipart/form-data">
		...
		<table>
			...
			<tr>
				...
				<td>
					<input type="text" name="settings[timeout]" value="<?php print $c['timeout']; ?>" id="timeout"
						   placeholder="300"/>
				</td>
			</tr>
			<tr>
				<td colspan="2">
					<input type="checkbox" name="settings[smtp-enabled]" value="1"
						   <?php print $c['smtp'] ? 'checked="checked" ' : ''; ?>id="smtp-enabled"/><label
			...
			<tr>
				...
				<td>
					<input type="text" name="settings[host]" value="<?php print $c['smtp']['host']; ?>"
						   id="smtp-hostname"/>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td>
					<input type="text" name="settings[username]" value="<?php print $c['smtp']['username']; ?>"
						   id="smtp-username"/>
				</td>
			</tr>
			<tr>
				...
				<td>
					<input type="password" name="settings[password]"
						   value="<?php print str_repeat('*', strlen($c['smtp']['password'])); ?>" id="smtp-password"/>
				</td>
			</tr>
			<tr>
				<td>
			...
			<tr>
				<td colspan="2">
					<input type="checkbox" name="settings[SingleTo]" value="1"
						   <?php print $c['SingleTo'] ? 'checked="checked" ' : ''; ?>id="smtp-singleto"/><label
			...
			<tr>
				...
				<td>
					<input type="text" name="settings[from_name]" value="<?php print esc_attr($c['from_name']); ?>"
						   id="from-name"/>
				</td>
			</tr>
			<tr>
				...
				<td>
					<input type="text" name="settings[from_email]" value="<?php print esc_attr($c['from_email']); ?>"
						   id="from-email"/>
				</td>
			</tr>
			<tr>
				<td colspan="2">
					<input type="checkbox" name="settings[spf_lookup_enabled]" value="1"
						   <?php print $c['spf_lookup_enabled'] ? 'checked="checked" ' : ''; ?>id="spf_lookup_enabled"/><label
						for="spf_lookup_enabled"><?php _e('Use SPF-lookup to determine validity of sender instead of domainname-match.', 'wpes'); ?></label>
				</td>
			</tr>
			...
			<?php if (!WP_Email_Essentials::i_am_allowed_to_send_in_name_of($c['from_email'])) { ?>
				...
				<td>SPF Records are checked: you are NOT allowed to send mail with this domain.<br/>
					If you really need to use this sender e-mail address, you need to change the SPF record to include
					the sending-IP of this server;<br/>
		...
		<?php if (!WP_Email_Essentials::i_am_allowed_to_send_in_name_of($c['from_email'])) { ?>
			<tr>
				<td></td>
				<td>You are NOT allowed to send mail with this domain; it should match the domainname of the
					website.<br/>
					If you really need to use this sender e-mail address, you need to switch to SPF-record checking and
					make sure the SPF for this domain matches this server.
				</td>
			...
			<tr>
				...
				<td>
					...
					<select name="settings[make_from_valid]" id="make_from_valid">
						<option
							value=""><?php _e('No, send with possibly-invalid sender as is. (might cause your mails to be marked as spam!)', 'wpes'); ?></option>
						<option disabled>────────────────────────────────────────────────────────────</option>
						<option value="-at-" <?php
			...
			<tr>
				<td colspan="2">
					<input type="checkbox" name="settings[is_html]" value="1"
						   <?php print $c['is_html'] ? 'checked="checked" ' : ''; ?>id="smtp-is_html"/><label
			...
			<tr>
				<td colspan="2">
					<input type="checkbox" name="settings[css_inliner]" value="1"
						   <?php print $c['css_inliner'] ? 'checked="checked" ' : ''; ?>id="smtp-css_inliner"/><label
 ...
 --> <?php if (function_exists('openssl_pkcs7_sign')) { ?>
				<tr>
					<td colspan="2">
						<input type="checkbox" name="settings[enable_smime]" value="1"
							   <?php print $c['enable_smime'] ? 'checked="checked" ' : ''; ?>id="enable-smime"/><label
				...
				<tr>
					...
					<td>
						<input type="text" name="settings[certfolder]"
			...
			<tr>
				<td colspan="2">
					...
					<?php _e('DEFAULTS can be overruled with WordPress filter <code>wpes_defaults</code>, parameters: <code>Array $defaults</code>', 'wpes'); ?>
					<br/>
					<?php _e('SETTINGS can be overruled with WordPress filter <code>wpes_settings</code>, parameters: <code>Array $settings</code>', 'wpes'); ?>
					<br/>
					<?php _e('Email HEAD can be overruled with WordPress filter <code>wpes_head</code>, parameters: <code>String $head_content</code>, <code>PHPMailer $mailer</code>', 'wpes'); ?>
					<br/>
					<?php _e('Email BODY can be overruled with WordPress filter <code>wpes_body</code>, parameters: <code>String $body_content</code>, <code>PHPMailer $mailer</code>', 'wpes'); ?>
					<br/>
				</td>
			</tr>
			<tr>
