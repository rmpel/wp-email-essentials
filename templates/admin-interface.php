	...
	<form id="outpost" class="wpes-admin" method='POST' action="" enctype="multipart/form-data">
		...
		<div class="wpes-tools">
			<div class="wpes-tools--box">
				...
				<a class="button action" href="#email-test" style="text-align: center">
					<?php esc_html_e( 'Send sample mail', 'wpes' ); ?>
				</a>
			</div>
		...
		<div id="poststuff">
					...
					<div class="inside">
						<div class="wpes-form">
							...
							<div class="wpes-notice--info on-enable-smime">
								<p>
									<?php
									print wp_kses_post( __( 'You can also type a relative path (any path not starting with a / is a relative path), this will be evaluated against ABSPATH (the root of your WordPress installation).', 'wpes' ) ) . '<br />';
									print wp_kses_post( __( 'The file-naming convention is', 'wpes' ) ) . ':<br />';
									?>
								</p>

								<label
									for="smime-sample-email"><?php esc_html_e( 'Type an email address to see the correct filenames', 'wpes' ) ?></label>
									...
									<tr>
										...
										<td>
											<code
												...
												data-default="e-mail@addre.ss">e-mail@addre.ss.crt</code>
										</td>
									</tr>
									<tr>
										<th>
										...
										<td>
											<code
												...
												data-default="e-mail@addre.ss">e-mail@addre.ss.key</code>
										</td>
									</tr>
									<tr>
										<th>
										...
										<td>
											<code
												...
												data-default="e-mail@addre.ss">e-mail@addre.ss.pass</code>
										</td>
									</tr>
								</table>
				...
				<div class="inside">
					<div class="wpes-form">
						...
						<div class="wpes-notice--info on-enable-dkim">
							<p>
								<?php print wp_kses_post( __( 'You can also type a relative path (any path not starting with a / is a relative path), this will be evaluated against ABSPATH (the root of your WordPress installation).', 'wpes' ) ); ?>
								<br/>
							...
							<label
								for="dkim-sample-domain"><?php esc_html_e( 'Type a domain to see the correct filenames and records', 'wpes' ) ?></label>
							<input type="text" id="dkim-sample-domain" class="dkim-sample-input">
							<label
								for="dkim-sample-domainkey"><?php esc_html_e( 'Type the desired domainkey to see the correct DNS records', 'wpes' ) ?></label>
							<input type="text" id="dkim-sample-domainkey" class="dkim-sample-input">
							<label
								for="dkim-sample-password"><?php esc_html_e( 'Type a sample password to see the correct scripts', 'wpes' ) ?></label>
							...
							<?php
							print wp_kses_post( '<code class="dkim-sample-output preload">openssl genrsa -aes256 -passout pass:"' . _x( 'YOUR-PASSWORD', 'A sample password', 'wpes' ) . '" -out domain.tld.key 2048</code>' );
							print wp_kses_post( '<code class="dkim-sample-output preload">openssl rsa -in domain.tld.key -pubout > domain.tld.crt</code>' );
							print wp_kses_post( '<code class="dkim-sample-output preload">echo "' . _x( 'YOUR-PASSWORD', 'A sample password', 'wpes' ) . '" > domain.tld.pass</code>' );
							print wp_kses_post( '<code class="dkim-sample-output preload">echo "' . _x( 'DKIM-SELECTOR-FOR-THIS-KEY', 'A sample DKIM selector', 'wpes' ) . '" > domain.tld.selector</code>' );
							print wp_kses_post( __( 'Upload these files to the specified path on the server and again; this should not be publicly queryable!!!', 'wpes' ) );
							?>
							<script>
								// items with class preload will generate pattern data on the fly
								var preload_data = [
									"domain.tld",
									<?php print json_encode( _x( 'DKIM-SELECTOR-FOR-THIS-KEY', 'A sample DKIM selector', 'wpes' ) ); ?>,
									<?php print json_encode( _x( 'YOUR-PASSWORD', 'A sample password', 'wpes' ) ); ?>
							...
							<?php print wp_kses_post( '<code class="dkim-sample-output preload" data-default1="" data-default2="" data-default3="" data-pattern="">' . _x( 'DKIM-SELECTOR-FOR-THIS-KEY', 'A sample DKIM selector', 'wpes' ) . '._domainkey.domain.tld. IN TXT "v=DKIM1; k=rsa; p=' . _x( 'CONTENT-OF', 'A tag that tells the user to get the content of a file', 'wpes' ) . '-domain.tld.crt"</code>' ); ?>
			...
			<div id="email-test" class="postbox">
				<div class="postbox-header">
					<h2>
						<?php print wp_kses_post( __( 'Send Test E-mail', 'wpes' ) ); ?>
					</h2>
				</div>
				<div class="inside cols">
					<div class="inside col">
						<strong><?php esc_html_e( 'Send test-email to:', 'wpes' ); ?></strong>
						<?php
						...
						$urls = [
							__( 'Blog settings', 'wpes' )      => admin_url( 'options-general.php' ),
							__( 'Site settings', 'wpes' )      => is_multisite() ? network_admin_url( 'settings.php' ) : false,
							__( 'Alternative Admins', 'wpes' ) => add_query_arg( 'page', 'wpes-admins', admin_url( 'admin.php' ) ),
						];
						$urls = array_filter( $urls );
						?>
						<em><?php esc_html_e( 'This setting will allow you to test the "Alternative Admins" settings.', 'wpes' ); ?></em>
					</div>
					<div class="inside col">
						<strong><?php esc_html_e( 'Send test-email from:', 'wpes' ); ?></strong>
						<?php
						$wpes_senders = [];
						// Add 'default' as set in the settings.
						$wpes_senders[ 'Default from e-mail: ' . $wpes_config['from_email'] ] = $wpes_config['from_email'];
						...
						?>
						<em><?php esc_html_e( 'This setting will allow you to test the sender-replacement setting. Depending on your settings above, the chosen address will either be the actual sender-address, or the reply-to address.', 'wpes' ); ?></em>
					</div>
				</div>
				<div class="inside">
					<input
