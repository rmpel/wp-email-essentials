...
<div class="wrap">
	...
	<form id="outpost" method='POST' action="">
		...
		<table>
			...
			<tbody>
			<?php foreach ($recipients as $recipient => $action) foreach ($keys as $key) foreach (array('post') as $post_type) {
				...
				<tr>
					<td>
						<label for="key-<?php print $post_type; ?>-<?php print $recipient; ?>-<?php print $key; ?>"><?php print $action; ?> to author on <?php print $key; ?> on <?php print $post_type; ?></label>
					</td>
					<td>
						<input type="text" name="settings[keys][<?php print $post_type; ?>][<?php print $recipient; ?>][<?php print $key ?>]"
							   placeholder="default: owner of <?php print $post_type; ?>" value="<?php print $c[$post_type][$recipient][$key]; ?>"
	...
	<p>Moderation for pingbacks and comments is: <?php print (get_option('moderation_notify') ? 'enabled' : 'disabled'); ?>. <a href="<?php print admin_url('options-discussion.php') ?>#comment_order">Change this setting</a>.</p>
</div>
<style>
	#outpost input[type=text] { width: 400px; }
</style>
